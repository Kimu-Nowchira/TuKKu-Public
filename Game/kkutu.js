...
exports.Client = function(socket, profile, sid){
	...
	*/
	...
	my.practice = function(level){
		...
		my.pracRoom.limit = 1;
		my.pracRoom.dictsize = $room.dictsize
...
exports.Room = function(room, channel){
	...
	my.setAI = function(target, level, team){
		...
		for(i in my.players){
			if(!my.players[i]) continue; // 플레이어가 아니면 패스
			if(!my.players[i].robot) continue; // 봇이 아니어도 패스
	...
	my.go = function(client, kickVote){
		...
		if(x == -1){
			client.place = 0;
			// 방 인원이 없어지면 방폭 (봇 제외하고)
			if(my.players.length < 1 && !my.botroom){
				console.log('방폭1==========================\n')
				delete ROOM[my.id]
			};
	...
	my.set = function(room){
		...
		my.round = Math.round(room.round);
		my.time = room.time * my.rule.time;
		my.dictsize = room.opts.dictsize
		my.botroom = room.botroom
		my.battery = room.battery
