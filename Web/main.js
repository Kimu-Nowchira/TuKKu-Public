...
function GameClient(id, url){
	...
	my.socket.on('open', function(){
		JLog.info(`${my.id}번 게임 서버에 연결되었어요`);
	});
	my.socket.on('error', function(err){
		JLog.warn(`${my.id}번 게임 서버에 다음과 같은 오류가 발생하였어요: ${err.toString()}`);
	});
	my.socket.on('close', function(code){
		JLog.error(`${my.id}번 게임 서버가 종료되었어요: ${code}`);
