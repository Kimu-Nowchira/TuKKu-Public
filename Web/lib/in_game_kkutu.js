...
function activeResize(){
	try{
		if($data.opts.nz){
			$("#Middle-Body").css('align-items', 'normal')
			$("body").css('overflow', 'auto')
			$(".size-fixed").css('zoom', 1);
			$("#Middle").css('zoom', 1);
			return;
		}
	}catch(e){};

	$("#Middle-Body").css('align-items', 'center')
	$("body").css('overflow', 'hidden')
	if($(window).height() > 100){
		$(".size-fixed").css('zoom', (700 / $(window).height() - 60));
		$("#Middle").css('zoom', ($(window).height() - 60) / 700);
	}
}
...
function hideDialog($d, noToggle){
...
function applyOptions(opt){
	...
	$("#no-zoom").attr('checked', $data.opts.nz);
...
function tryMainDict(text, callback){
	if(text.startsWith('/')){
		var text = text.replace(/[^\sa-zA-Zㄱ-ㅎ0-9가-힣]/g, "");
		$.get("/lettermake/" + text, callback);
		return;
	}
	var text = text.replace(/[^\sa-zA-Zㄱ-ㅎ0-9가-힣]/g, "");
	var lang = text.match(/[ㄱ-ㅎ가-힣]/) ? 'ko' : 'en';
	...
	if(text.length < 1) return callback({ error: 404 });
	$.get("/dicts/" + text + "?lang=" + lang, callback);
}
...
*/
	...
	$stage.box.me.show();
	$stage.box.chat.show().width(800).height(200);
	...
	if(only == "for-lobby"){
		...
		$data._shop = false;
		$stage.box.room.show().height(370);
		if(only == "for-master") if($stage.dialog.inviteList.css("visibility") == 'visible') updateUserList();
		updateRoom(false);
		updateMe();
...
function checkRoom(modify){
	...
	if($data._gaming != $data.room.gaming){
		if($data.room.gaming){
			...
			if($data._spectate){
				hideDialog($stage.dialog.resultSave)
...
function updateRoom(gaming){
	...
	if($data.room.players.length > 8) $(".game-body").css('justify-content', 'left')

	if($stage.dialog.profile.css("visibility") == 'visible'){
...
function gameReady(){
	...
	$data._gAnim = true;
	$stage.box.room.show().height(370).animate({ 'height': 1 }, 500);
	$stage.box.game.height(1).animate({ 'height': 420 }, 500);
	stopBGM();
	...
	forkChat();
	addTimeout(function(){
		$stage.box.room.height(370).hide();
...
function toDict(words){
	var $R = $("<li>").addClass("search_result").css('zoom', 700 / ($(window).height() - 60));
	// 단어 단위
	words.forEach(function(val, idx){
		...
		$L.append($("<dt>").append($("<span>").addClass("search_word").html(word)));
		...
		means.forEach(function(m1, x1){
			...
			m1.forEach(function(m2, x2){
				...
				m2.forEach(function(m3, x3){
					...
					// 주제 구하기
					var $meanbox = $("<dd>")
					$meanbox.append($("<span>").addClass("search_word_detail").html(word));
					$meanbox.append($("<span>").addClass("word_number").html(index));
					index += 1
					if(_t) $meanbox.append($("<span>").addClass("word_theme").html(_t));
					$meanbox.append($("<span>").addClass("word_mean").html(formMean(m3)));
					$L.append($meanbox);
					// $m2.append($m3);
				});
				// $m1.append($m2);
	...
	means.forEach(function(m1, x1){
		...
		m1.forEach(function(m2, x2){
			...
			m2.forEach(function(m3, x3){
				...
				var theme_group = 1
				// 주제 구하기
				if(_t) $m3.append($("<label>").addClass("word-theme-" + theme_group).html(_t));
...
function setRoomHead($obj, room){
	...
	$obj.empty()
		...
		.append($rm = $("<h5>").addClass("room-head-mode").html(opts.join(" / ") + ' / ' + L['dict_' + room.dictsize]))
...
function playBGM(key, force, vol){
	if($data.bgm) $data.bgm.stop();
	
	return $data.bgm = playSound(key, true, vol);
}
function stopBGM(){
	if($data.bgm){
		$data.bgm.stop();
...
function playSound(key, loop, vol){
	var src, sound;
	...
	if(window.hasOwnProperty("AudioBuffer") && sound instanceof AudioBuffer){
		...
		if(sound.readysState) sound.audio.currentTime = 0;
...
function queueObtain(data){
	if($stage.dialog.obtain.css("visibility") == 'visible'){
