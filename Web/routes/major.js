...
Server.get("/dicts/:word", function(req, res){
    var word = req.params.word.replace("'", '');
    var DB = MainDB.kkutu["ko"];
    DB.find([ '_id', new RegExp(`^${word}.*`) ]).sort(['length(_id)', 1]).limit(15).on(function($words){
        if(!$words) return res.send({ error: 404 });
        res.send($words);
	});
});
Server.get("/lettermake/:letters", function(req, res){
    var letters = req.params.letters.replace("'", '');
    var DB = MainDB.kkutu["ko"];
    DB.find([ '_id', new RegExp(`^[${letters}]+$`) ]).sort(['length(_id)', -1]).limit(100).on(function($result){
